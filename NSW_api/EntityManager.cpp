...
std::vector< EItem::item_action_struct*> EItem::ITEM_ACTION_REGISTERER;

Entity* EItem::create_bullet(Entity* _e, EItem* _item, float _target_x, float _target_y, float _d)
{
	Entity* bullet = new Entity();
	*bullet->position_x = *_e->position_x;
	*bullet->position_y = *_e->position_y;
	...
	*bullet->light_source_blue = 0.7f;
	...
	float ray_length_x = _target_x - *_e->position_x;
	float ray_length_y = _target_y - *_e->position_y;
	...
	float mul_x = new_x / dst;
	float mul_y = new_y / dst;
...
Entity* EItem::shoot_bullet(Entity* _e, EItem* _item, float _target_x, float _target_y, float _d)
{
	for (int i = 0; i < *_item->item_attributes_list.at(EItem::ItemAttribute::ITEM_ATTRIBUTE_BULLET_COUNT)->item_total_attr; i++)
	{
...
void EItem::item_action_default_shoot(EItem* _item, float _d)
{
}
